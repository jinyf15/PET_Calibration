function forward_proj_PET_multiang_display_new_v2(x, fitIndex,syspara,total_data,iSave)
global videonameTotal
global resultfolder
fullX(fitIndex==1) = x;
x = fullX;
...
%'done2'
    for na=0:syspara.NANG-1
        ...
        msyspara.mdata=total_data{na+1};
        ...
        if iSave
                ...
                if syspara.iter == 1
                    videonameTotal=[resultfolder,'FinalResults',datestr(now,30),'.gif'];
                    imwrite(imind,cm,videonameTotal,'gif', 'Loopcount',inf,'DelayTime', 0.5);
                else
                    imwrite(imind,cm,videonameTotal,'gif','WriteMode','append');
                end
        end
        %view(15, 30);
        %view([90 0]);
