function pr=forward_proj_PET_multiang_new_v3(x, syspara,fitIndex,fullX,total_data)
fullX(fitIndex==1) = x;
x = fullX;
iSave = 1;
det_c = zeros(3,16);
...
for na=0:syspara.NANG-1
    ...
    l = size(total_data{1},2);
    for it=1:l
        syspara.mdata{:} = total_data{:}(:,it);
        data=syspara.mdata{na+1};
        pr = forward_proj_multi_angle_prob(data, size(data,2), size(sp,1), dp, dpc, sp(:,1),sp(:,2),sp(:,3),...
        ...
        txt = ['line',num2str(it),' prob=',num2str(pr)];
        title(txt); hold on;
        ...
        subplot(1,2,2);
        plot(it,pr,'.','Markersize',30,'color','r'); 
        hold on
        xlabel('line');ylabel('log(pr)');xlim([1 l]);
        if iSave
                drawnow
                videoname=['../data/videoScan3.gif'];
                frame = getframe(gcf);
                im = frame2im(frame);
