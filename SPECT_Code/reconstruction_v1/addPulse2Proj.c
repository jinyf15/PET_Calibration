...
void addPulse2Proj()
{
    ...
    for(ii = 0; ii < nIMG; ii++)
    {
        if(ii==Img.NZ / 2.0 * (Img.NX*Img.NY) + Img.NY / 2.0 * Img.NX + Img.NX/2.0)  // add a pulse in the center of FOV;
            imgTMP[ii] = imgTMP[ii] * 0.05;
        else
            imgTMP[ii] = 0;
    }
    ...
    for(iSouP = 0;iSouP < nSouP; iSouP++)
    {
        for(iDet = 0; iDet < nDet; iDet++)
        {
            ...
            ii = iSouP * nDet + iDet;
            sprintf(fileName,"%s//proj_iDet%d_iPosi%d_withPulse",&(Proj[0].projFolder[0]),iDet,iSouP);  // mean projection
...
//            {
//                jj = ii*dN + iPix;
//                Proj[ii].detImage[iPix] = (double) poidev( (float) Proj[ii].detImage[iPix], &(randSeed[jj]));
//            }
//
//            sprintf(fileName,"%s//proj_iDet%d_iPosi%d",&(Proj[0].projFolder[0]),iDet,iSouP);  // mean projection
//            fp1 = fopen(fileName,"wb");
//            fwrite( &( Proj[ii].detImage[0] ), sizeof(double), dN,fp1);
//            fclose(fp1);
