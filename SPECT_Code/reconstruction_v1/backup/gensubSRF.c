...
int genSubSRF(struct detector * det,int iDet,int iSubDet,struct source *sou,int iSouP,struct pinMap *ph,struct sysSRF *srf,int iThread)
{
	...
	unsigned long int sn; // index of source voxel;
	int phIndex;
	char fileName[1000];
	FILE *fp1;
    ...
    thresh = 1.e-12;
    cos_D = (double *)malloc(sizeof(double)*DM_DET);/*/incident angle;/*/
	...
	for(sn=0; sn<NIMG;sn++)
	{
        ...
        {
            time(&(time1));
            printf("fininshed %ld,total %ld,srf size %ld, maxiumusize %ld,time %s\n",sn,NIMG,sprk,srf[iThread].nMax,asctime(localtime(&time1)));
        }
            ...
            for(ndy=ntmp3; ndy<=ntmp4; ndy++)
            {
                for(ndx=ntmp5; ndx<=ntmp6; ndx++)
                {
                    ...
                    if ((det_pix_mask[dn]==nsubdet)||(det_pix_mask[dn]==100+nsubdet))
                    {
                        ...
                        //pr=pixelprob_det2ML_sum_ph_insert2_double10(buf, dpos, subdpos, spos, ph[phIndex].map, N_step, cos_D, DM_DET, MU_DET, fn);
                        index111=index111+1;
                        if(pr>thresh)
                        {
        ...
        if(0)
        {
            sprintf(fileName,"%s//proj_sn%d_inph%d",DATA_DIR,sn,k);
            printf("fileName,%s",fileName);
            fp1 = fopen(fileName,"wb");
            fwrite(proj,sizeof(double),DN_DETX1*DN_DETY1,fp1);
            fclose(fp1);
            printf("save psf to %s \n",fileName);
            getchar();
        }
