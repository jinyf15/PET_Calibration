...
void linbcg1_multipoints_eff2_filter(unsigned long n, unsigned long m, double b[], double x[], int itol, float tol,
	int itmax, int *iter, float *err, int fwhm, int pt)
{
	void atimes_multipoints_diag2(unsigned long mm, float x[], float rr[], float diag[], int itrnsp);
	void atimes_multipoints_22(unsigned long mm, float x1[], float x2[], float rr1[], float rr2[], int itrnsp);
	float snrm1_multipoints(unsigned long n, double sx[], int itol);
	unsigned long j, i;
	...
	*iter=0;
	atimes_multipoints_diag2(n,x,r, diag_tmp, 1);
	...
	while (*iter <= itmax) {
		...
		bkden=bknum;
		atimes_multipoints_22(n,p,pp,z,zz,1);
		...
		if ((*iter/100*100)==*iter){
			for (j=1; j<=n; j++) g[j]=0;
			atimes_multipoints_22(n, x, x, g, g, 0);

			sprintf(filename, "%s\\results\\x%d_%d_%d", ROOT_DIR, pt, fwhm, *iter);
			fp1=fopen(filename, "wb"); 
			for (j=1; j<=n; j++) fwrite(&(x[j]), sizeof(double), 1, fp1);			
			fclose(fp1);

			sprintf(filename, "%s\\results\\g%d_%d_%d", ROOT_DIR, pt, fwhm, *iter);
			fp1=fopen(filename, "wb"); 
