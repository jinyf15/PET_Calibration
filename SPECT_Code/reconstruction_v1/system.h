...
#define PC_MAX_MEMORY 320 // be cautious to change this value; if using too large value, pc will be dead;
#define MAX_CPU 64
...
struct parellSequence
{
    ...
    int nSubSet; // number of sebsut in OSEM;
    int iSubSet;

    int nThread; // number of thread;
    int iThread; // which thread this squence belong;

    int nFinished; // # of load finished;
    int nLoad; // # of sequence belong to ithread; in reconstruction nLoad means how many calculation load in each iter for all thread;
...
struct sysSRF
{
    ...
    unsigned long int mSize; // size of subSRF;
...
struct projection
{
    ...
    int iPosi; // which detector it is
    int nSubDet; // number of sub detector pixel
    int NX,NY; // number of detector pixel in each demension
    int *detPixMask; // detector pixelMask
    double *detImage;
    char projFolder[1000]; // the folder of save projection;
    int iProj;
};
