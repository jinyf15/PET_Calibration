...
void linbcg1_multipoints_eff(unsigned long n, unsigned long m, float b[], float x[], int itol, float tol,
	int itmax, int *iter, float *err)
{
	void atimes_multipoints_diag(unsigned long mm, float x[], float rr[], float diag[], int itrnsp);
	void atimes_multipoints_2(unsigned long mm, float x1[], float x2[], float rr1[], float rr2[], int itrnsp);
	void atimes_multipoints_2_parallel(unsigned long mm, float x1[], float x2[], float rr1[], float rr2[], int itrnsp);
	void atimes_multipoints_2_parallel_master(unsigned long mm, float x1[], float x2[], float rr1[], float rr2[], int itrnsp);
	void atimes_multipoints_2_parallel_slave(unsigned long mm);	
...
/////////////////////////////////////////////////////////////////////////*/
	*iter=0;
	atimes_multipoints_diag(n,x,r, diag_tmp, 1);
...
/**************************************************/
		...
		sprintf(filename2, "%s\\data\\bknum_%d", ROOT_DIR, *iter);
		if (if_period) {
			while (1){
				_ftime( &tstruct );
		...
		//printf("in!!!!!!!!!!!1\n");
		atimes_multipoints_2_parallel(n,p,pp,z,zz,1, *iter);
		*iter=niter;
		//printf("out!!!!!!!1\n");
...
//		nn=0; while ((remove(filename2) == -1 ) && ((nn++)<100)) printf("delete error: %s\n", filename2);
		...
		while (1) {
			sprintf(filename2, "%s\\data\\x12_%d", ROOT_DIR, niter);
			if (if_period) {
				while (1){
					_ftime( &tstruct );
					if (tstruct.millitm/time_period*time_period==tstruct.millitm-time_period/2){
