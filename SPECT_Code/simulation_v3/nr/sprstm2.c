...
void sprstm2(float sa[], unsigned long ija[], float sb[], unsigned long ijb[],
	float thresh, unsigned long nmax, float sc[], unsigned long ijc[], int limita, int limitb)
{
...
/*			for (mbb=mb;mbb<=ijb[j+1]-1;mbb++) {
	...
	for (i=min(limita, limitb)+1; i<=ija[1]-2; i++) {
		ijc[i+1]=ijc[min(limita, limitb)+1];
	}
}
