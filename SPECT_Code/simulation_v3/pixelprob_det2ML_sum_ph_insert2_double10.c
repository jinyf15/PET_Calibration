...
//        beta_phi = acos(ny);  // this  needs to be very careful. phi is based on x asix;it has to be consistent with penetration map generation sequence
//		if(nx < 0) beta_phi = 2*PI-beta_phi;
		...
		if (1) {
			...
			for(ii=0; ii<fn; ii++) {
				for(jj=0; jj<fn; jj++) {
					...
					Lindex++;
					if ((r>=Rlimit) || (fabs(rx)>=(N1/2-1)*dirt[0]) || (fabs(ry)>=(N2/2-1)*dirt[1])) { /*/// outside of the rho limit/*/
						continue;
					}
					else{
						tmpi = rx/dirt[0]+N1/2;
